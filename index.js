...
async function run() {
  try {
    ...
    app.post("/allTeddy", async (req, res) => {
      ...
      newTeddy.price = parseFloat(req.body.price)
    ...
    // this query for getting data from category

    app.get("/newTeddy/:text", async (req, res) => {
      if (
        req.params.text == "Honey Bear" ||
        req.params.text == "Snugglekins" ||
        req.params.text == "Fuzzy"
      ) {
    ...
    app.put("/myTeddy/:id", async (req, res) => {
      ...
      const teddy = {
        $set: {
          price: parseFloat(udpdatedTeddy.price),
